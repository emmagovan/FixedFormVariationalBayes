...
library(R2jags)
library(progress)
wolves <- read.csv("wolves_consumer.csv", header = TRUE, stringsAsFactors = FALSE)
```
...
plot(sigma_sq[3, ])
plot(h_theta)
plot(log_q_lambda_theta)
plot(delta_lqlt[, 1])
plot(LB[2, , 4])
plot(h_theta - log_q_lambda_theta)
#x <- seq(-10, 10, length = length(mu_mu)) * sqrt(mean(sigma_sq_mu)) + mean(mu_mu)
#hx <- rnorm(2000, mean(mu_mu[4000:5000]), mean(sqrt((sigma_sq_mu[4000:5000]))))
#ffvbmean<-plot(x, hx)
...
#ffvbsigma<-plotInvGamma(((mean(alpha_sig[8000:9999]))), ((mean(beta_sig[8000:9999]))))
# png("sigma_is_converging.jpeg")
#mean(beta_sig / (alpha_sig - 1))

# dev.off()#mean of sigma^2
```
...
#mu is approx -20.9 - similar to above
#sigma is 2.27 - again similar
#rhat values all look good - 1.003 is the highest
...
mu looks good
Sigma seems a bit too high
```{r}
mycol <- rgb(0, 0, 255, max = 255, alpha = 125, names = "blue50")
mycol2 <- rgb(255, 0, 0, max = 255, alpha = 125)

#png("FFVB_vs_JAGS.jpeg")
...
#sample<-rinvgamma(8000, shape = mean(alpha_sig[(n_iter-5000):n_iter]), rate = mean(beta_sig[(n_iter-5000):n_iter]))

samplegamma<-rgamma(8000, shape = mean(alpha_sig[(n_iter-5000):n_iter]), rate = mean(beta_sig[(n_iter-5000):n_iter]))


#png("tau_FFVB_JAGS.jpeg")
#hist((model_run$BUGSoutput$sims.list$sigma), col= mycol2, breaks = 10)
#hist(sqrt(sample), add=TRUE, col=mycol, breaks =20)
#dev.off()
...
#hist(sample, xlim=c(0, 10), breaks = 10)
