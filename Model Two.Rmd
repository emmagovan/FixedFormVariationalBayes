...
## Introduction
This file contains the maths and code for running a model through a Fixed Form Variational Bayes Algorithm.


## Model
$y \sim N(\mu, \sigma^2 + 3)$

$\mu \sim N(\mu_\mu, \sigma^2_\mu)$

$\sigma^2 \sim InvGa(\alpha_{\sigma^2}, \beta_{\sigma^2})$

The priors for mu and sigma are Normal and inverse Gamma respectively. We we use the same distributions for the variational posteriors.


## Variational Inference
Maths for this algorithm: from [here](https://vbayeslab.github.io/VBLabDocs/tutorial/ffvb)
...
$$h_\lambda(\theta) = log\left(\frac{p(y|\mu, \sigma)p(\mu)p(\sigma)}{q(\mu)q(\sigma)}\right)$$
...
model_parameters = c('mu', 'tau', 'sigmasq')
...
&=& -\frac{n+1}{2}\log{2\pi} - 0.5\log(\sigma^2 + 3) +\alpha_0\log(\beta_0) -\log\Gamma(\alpha_0)-(\frac{n}{2}+\alpha_0+1)\log(\sigma^2 +3) -\frac{\beta_0}{\sigma^2+3}-\frac{1}{2(\sigma^2+3)}\sum_{i=1}^n(y_i-\mu)^2\\
...
sigma_sq[1, ] <- rinvgamma(S, alpha_sig[1], scale = beta_sig[1])
...
for (i in 2:n_iter) {
  ...
  for (j in 1:S) {
    ...
    h_theta[i] <- -(((n + 1) / 2) * log(2 * pi)) - (0.5 * log(sigma_sq_mu[1])) - (((mu[i, j] - mu_mu[1])^2) / (2 * sigma_sq_mu[1])) + (alpha_sig[1] * log(beta_sig[1])) - (log(gamma(alpha_sig[1]))) - ((n / 2 + alpha_sig[1] + 1) * log(sigma_sq[i, j] +3)) - (beta_sig[1] / (sigma_sq[i, j] + 3)) - ((1 / (2 * (sigma_sq[i, j] +3))) * (sum((y - mu[i, j])^2)))

    log_q_lambda_theta[i] <- (alpha_sig[i - 1] * log(beta_sig[i - 1])) - log(gamma(alpha_sig[i - 1])) - ((alpha_sig[i - 1] + 1) * log(sigma_sq[i, j])) - (beta_sig[i - 1] / sigma_sq[i, j]) - (0.5 * log(2 * pi)) - (0.5 * log(sigma_sq_mu[i - 1])) - (((mu[i, j] - mu_mu[i - 1])^2) / (2 * sigma_sq_mu[i - 1]))

    delta_lqlt[i, ] <- c((mu[i, j] - mu_mu[i - 1]) / sigma_sq_mu[i - 1], -(1 / (2 * sigma_sq_mu[i - 1])) + (((mu[i, j] - mu_mu[i - 1])^2) / (2 * (sigma_sq_mu[i - 1])^2)), log(beta_sig[i - 1]) - digamma(alpha_sig[i - 1]) - log(sigma_sq[i, j]), (alpha_sig[i - 1]) / (beta_sig[i - 1]) - (1 / (sigma_sq[i, j])))
  ...
  beta_sig[i] <- beta_sig[i - 1] + 0.005 * (gb[i]/(sqrt(vb[i])))
}
...
#Generate mu sample
hx <- rnorm(8000, mean(mu_mu[(n_iter-1000):n_iter]), sqrt(mean((sigma_sq_mu[(n_iter-1000):n_iter]))))
...
samplesigmasq<-rinvgamma(8000, shape = alpha_sig[n_iter], rate = beta_sig[n_iter])


#Plot JAGS vs VB
hist(model_run$BUGSoutput$sims.list$sigmasq, col= mycol2, breaks = 100)
hist(samplesigmasq, col=mycol, breaks =1000,  add = TRUE)
...
Again this model is working well and seems to be matching both mu and sigma^2
Next model - I will look at 1 isotope, but kept very simple - no TEFs, corrections etc
