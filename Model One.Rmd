...
for (i in 2:n_iter) {
  ...
  for (j in 1:S) {
    ...
    h_theta[i] <- -(((n + 1) / 2) * log(2 * pi)) - (0.5 * log(sigma_sq_mu[1])) - (((mu[i, j] - mu_mu[1])^2) / (2 * sigma_sq_mu[1])) + (alpha_sig[1] * log(beta_sig[1])) - (log(gamma(alpha_sig[1]))) - ((n / 2 + alpha_sig[1] + 1) * log(sigma_sq[i, j])) - (beta_sig[1] / sigma_sq[i, j]) - ((1 / (2 * sigma_sq[i, j])) * (sum((y - mu[i, j])^2)))
...
samplesigma<-rgamma(8000, shape = mean(alpha_sig[(n_iter-5000):n_iter]), rate = mean(beta_sig[(n_iter-5000):n_iter]))


#Plot JAGS vs VB
hist((model_run$BUGSoutput$sims.list$tau), col= mycol2, breaks = 10)
hist((samplesigma), add=TRUE, col=mycol, breaks =10)
